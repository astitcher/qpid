...
bool tokenise(std::string::const_iterator& s, std::string::const_iterator& e, Token& tok)
{
    ...
    case START:
        ...
        case '(': type = T_LPAREN; state = ACCEPT_INC; continue;
        case ')': type = T_RPAREN; state = ACCEPT_INC; continue;
        case ',': type = T_COMMA; state = ACCEPT_INC; continue;
        case '+': type = T_PLUS; state = ACCEPT_INC; continue;
        case '-': type = T_MINUS; state = ACCEPT_INC; continue;
        case '*': type = T_MULT; state = ACCEPT_INC; continue;
        case '/': type = T_DIV; state = ACCEPT_INC; continue;
        case '=': type = T_EQUAL; state = ACCEPT_INC; continue;
        case '<':
            ++t;
            if (t==e || (*t!='>' && *t!='='))
                {type = T_LESS; state = ACCEPT_NOINC; continue; }
            else
                {type = (*t=='>') ? T_NEQ : T_LSEQ; state = ACCEPT_INC; continue; }
        case '>':
            ++t;
            if (t==e || *t!='=')
                {type = T_GRT; state = ACCEPT_NOINC; continue;}
            else
                {type = T_GREQ; state = ACCEPT_INC; continue;}
        default:
            break;
        }
    ...
    case EXPONENT_START:
        ...
        continue;
    case DIGIT:
        if (t==e) {type = T_NUMERIC_EXACT; state = ACCEPT_NOINC;}
        else if (std::isdigit(*t)) {++t; state = DIGIT;}
        else if (*t=='.') {++t; state = DECIMAL;}
        else if (*t=='e' || *t=='E') {++t; state = EXPONENT_SIGN;}
        else {type = T_NUMERIC_EXACT; state = ACCEPT_NOINC;}
        continue;
    case DECIMAL:
        if (t==e) {type = T_NUMERIC_APPROX; state = ACCEPT_NOINC;}
        else if (std::isdigit(*t)) {++t; state = DECIMAL;}
        else if (*t=='e' || *t=='E') {++t; state = EXPONENT_SIGN;}
        else {type = T_NUMERIC_APPROX; state = ACCEPT_NOINC;}
        continue;
    case EXPONENT:
        if (t==e) {type = T_NUMERIC_APPROX; state = ACCEPT_NOINC;}
        else if (std::isdigit(*t)) {++t; state = EXPONENT;}
